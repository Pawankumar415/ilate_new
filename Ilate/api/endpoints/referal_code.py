...
#         existing_code = db.query(ReferralCode).filter(ReferralCode.code == referral_data.code).first()
#         if existing_code:
#             raise HTTPException(status_code=400, detail="Referral code already exists")

#         expiry_days = referral_data.expiry_days or 2
#         discount_rupees = referral_data.discount_rupees or 0
...
#         expiry_date = new_code.created_on + timedelta(days=expiry_days)
...
async def validate_referral_code(code: str = Query(..., description="Enter the referral code to validate"), db: Session = Depends(get_db), current_user: LmsUsers = Depends(get_current_user)):
    try:
        ...
        return {
            ...
            "discount_multiplier": code_entry.discount_rupees,
            "code": code_entry.code,
            # "expiry_date": expiry_date_ist.isoformat(), # Removed
