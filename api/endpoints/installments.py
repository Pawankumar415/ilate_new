...
async def post_payment_details(
    ...
    try:
        ...
        existing_installments = db.query(Installment).filter(Installment.payment_id == payment_id).first() # added by bhavan kumar
        if existing_installments:
            raise HTTPException(status_code=400, detail="Installments already created for this payment")
